...
export class TipoEmpleoService {
    ...
    getAll() {
        return this.http.get(this.config.apiUrl + 'personas/tiposempleo', (response: Response) => response.json());
    }

    getByTipo(id: number) {
        return this.http.get(this.config.apiUrl + 'personas/tiposempleo/' + id, (response: Response) => response.json());
    }
