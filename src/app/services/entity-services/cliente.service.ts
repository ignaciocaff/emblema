...
export class ClienteService {
    ...
    getAll() {
        return this.http.get(this.config.apiUrl + 'clientes', (response: Response) => response.json());
    }
    ...
    getByDoc(nroDoc: number) {
        return this.http.get(this.config.apiUrl + 'personas/documento/' + nroDoc, this.jwt()).map((response: Response) => response.json());
    }
