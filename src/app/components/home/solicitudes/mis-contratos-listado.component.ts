...
@Component({
    moduleId: module.id,
    selector: 'mis-contratos-listado',
    templateUrl: 'mis-contratos-listado.component.html',
    styleUrls: [
        './mis-contratos-listado.component.css'
    ],
    providers: [ContratoService]
})

export class MisContratosListadoComponent implements AfterViewInit {
    ...
    cargarListado(params: any) {
        this.blockUI.start();
        if (this.empleado.cargo.n_cargo == 'Supervisor') {
                    ...
                    data => {
                        if (data) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                if (this.empleado.id_empleado == data[i]["supervisor_venta"]["id_empleado"]) {
        ...
        } else if (this.empleado.cargo.n_cargo == 'Asesor') {
            if (this.contrato.id_contrato > 0) {
                this.contratoService.getByContrato(this.contrato.id_contrato).subscribe(
                    data => {
                        if (data) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                ...
                                if (this.empleado.id_empleado == data[i]["asesor"]["id_empleado"]) {
                                    let solJSON = new Array<{
                                        id_contrato: number,
                                        id_supervisor_venta: string,
                                        nombreS: string,
