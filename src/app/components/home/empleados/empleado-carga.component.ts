...
export class EmpleadoCargaComponent {
    ...
    getSupervisores() {
        this.empleadoService.getByCargo(1).subscribe(

            data => {
                for (var i = 0; i < data.json().length; i++) {
                    let supervisor = new Empleado();
                    supervisor.id_empleado = data.json()[i]["id_empleado"];
                    this.personaService.getById(data.json()[i]["id_persona"]).subscribe(
                        data2 => {
                            for (var i = 0; i < data2.json().length; i++) {
                                supervisor.nombre = data2.json()[i]["nombre"];
                                supervisor.apellido = data2.json()[i]["apellido"];
                                }
                                this.lsEmpleados.push(supervisor);
                        },
                        error => {
                        }
                    )
